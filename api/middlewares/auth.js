...
async function validateServiceKey(req, res, next) {
  ...
  if (!serviceKey) return res.status(400).json({ status: "failure", tokenValid: false, message: "Service key missing" });

  const service = await Service.findOne({ where: { service_key: serviceKey, is_active: true } });
  if (!service) return res.status(404).json({ status: "failure", tokenValid: false, message: "Service not found" });
